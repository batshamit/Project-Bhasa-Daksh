...
def init_database():
    ...
    # Add preferred_language column if existing table lacks it
    try:
        cursor.execute("ALTER TABLE users ADD COLUMN preferred_language TEXT NOT NULL DEFAULT 'English';")
...
def seed_default_data():
    ...
    # Seed users
    users = [
        ('admin', 'admin123', 'Administrator', 'admin', 'approved', 'English'),
        ('student1', 'pass123', 'Rahul Kumar', 'student', 'approved', 'English'),
        ('student2', 'pass123', 'Priya Sharma', 'student', 'approved', 'Hindi'),
        ('student3', 'pass123', 'Amit Patel', 'student', 'approved', 'Hindi')
    ]
    for u in users:
        cursor.execute("INSERT OR IGNORE INTO users (username, password, name, role, status, preferred_language) VALUES (?, ?, ?, ?, ?, ?)", u)
...
def add_user(username, password, name, role='student', status='pending', preferred_language='Hindi'):
    """Adds a new user. Returns True if added, False if username exists."""
    conn = get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("INSERT INTO users (username, password, name, role, status, preferred_language) VALUES (?, ?, ?, ?, ?, ?)", 
                       (username, password, name, role, status, preferred_language))
        conn.commit()
        return True
...
def get_user(username):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT username, password, name, role, status, preferred_language FROM users WHERE username=?", (username,))
    row = cursor.fetchone()
    conn.close()
    return dict(row) if row else None

def update_user_language(username, language):
    """Updates preferred language for a user."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("UPDATE users SET preferred_language=? WHERE username=?", (language, username))
    conn.commit()
    conn.close()

def set_course_language(username, course_id, language):
    """Sets course specific language preference for a user."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute('''
        INSERT OR REPLACE INTO course_preferences (username, course_id, language)
        VALUES (?, ?, ?)
    ''', (username, course_id, language))
    cursor.execute("UPDATE users SET preferred_language=? WHERE username=?", (language, username))
    conn.commit()
    conn.close()

def get_course_language(username, course_id):
    """Retrieves course specific language preference for a user."""
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT language FROM course_preferences WHERE username=? AND course_id=?", (username, course_id))
    row = cursor.fetchone()
    if row and row['language']:
        lang = row['language']
        conn.close()
        return lang
    cursor.execute("SELECT preferred_language FROM users WHERE username=?", (username,))
    row_user = cursor.fetchone()
    conn.close()
    if row_user and row_user['preferred_language']:
        return row_user['preferred_language']
    return None
...
def get_pending_users():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT username, password, name, role, status, preferred_language FROM users WHERE status='pending'")
...
def get_all_students():
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT username, password, name, role, status, preferred_language FROM users WHERE role='student' AND status='approved'")
    rows = cursor.fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_enrolled_students(course_id):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT u.username, u.name, u.preferred_language,
               COALESCE(cp.language, u.preferred_language) AS course_language
        FROM users u
        JOIN enrollments e ON u.username = e.username
        LEFT JOIN course_preferences cp ON u.username = cp.username AND cp.course_id = e.course_id
...
def get_best_scores(username):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT module_id, MAX(score) as best_score, total, pls_phase, timestamp 
